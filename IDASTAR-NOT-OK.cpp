...
public:
    ...
    // Busca com profundidade limitada, retornando o menor limite e a solução se encontrada
    pair<int, vector<Node*>> depthLimitedSearch(Node* node, int g_cost, int limit) {
        ...
        int next_limit = INT_MAX;  // Inicialmente, o próximo limite é infinito

        for (Node* child : children) {
            if (child->parent && child->parent->state == child->state) {
                continue;  // Evitar estados do pai
            }         

            child->g_cost = g_cost + 1;
            child->h_cost = manhattanDistance(child->state, goal_state);
            

            auto result = depthLimitedSearch(child, child->g_cost, limit);

            if (!result.second.empty()) {  // Se a solução foi encontrada
                return result;
            }

            next_limit = min(next_limit, result.first);  // Menor valor para o próximo limite
        }

        return {next_limit, {}};  // Retornar o próximo limite e uma solução vazia
    }

    // Implementação do IDA* com incremento de limite correto
    vector<Node*> idaStar(const vector<int>& initial_state) {
        ...
        while (solution_path.empty()) {  // Repetir até encontrar uma solução
            auto result = depthLimitedSearch(initial_node, 0, limit);
...
int main() {
    vector<vector<int>> puzzle_instances = {
        {0, 6, 1, 7, 4, 2, 3, 8, 5},  // Instância problemático
    };

    vector<int> goal_state = {0, 1, 2, 3, 4, 5, 6, 7, 8};

    testIDAStar(puzzle_instances, goal_state);
